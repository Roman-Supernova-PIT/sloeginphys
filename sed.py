...
class SED:
    ...
    def append(self, lamb, flam, func=None, cont=None, npix=None):
        ...
        if np.shape(flam) != lshape:
            print('Shape of lamb and flam do not match.')
            return
        else:
            if lshape:
                nadd = lshape[0]
    ...
    def __call__(self, wave, fnu=False, **kwargs):
        ...
        wave : float, int, or `np.ndarray`
           The wavelength values in Angstrom
    ...
    def from_HDU(cls, hdu):
        ...
        if units in ('angstrom', 'angstroms', 'a', ''):
            ...
            print(f'Unknown wavelength units {units}. Assuming A')
        ...
        if units in ('fnu',):
            ...
            flam = np.copy(flux)
            flamunc = np.copy(func)
            print(f'Unknown flux units {units}.  Assuming Flam')
    ...
    def from_file(cls, filename, exten=1):
        ...
        if os.path.exists(filename):
            ...
            if ext in ('dat', 'txt', 'ascii', 'sed', ''):
                lamb, flam = np.loadtxt(filename, usecols=(0, exten), unpack=True)
                obj = cls(lamb, flam)
            ...
            print(f"SED File not found: {filename}")
            return None
    ...
    def get_from_CDBS(atlas, filename):
        ...
        if atlas == 'bc95':
            ...
            print(f'Invalid CDBS Atlas: {atlas}.  Skipping download.')
            return
        ...
        try:
            ...
            print(f"Unable to retrieve server file: {remotefile}")
            return
