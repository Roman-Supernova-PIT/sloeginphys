...
Overlapping pixels - In general, overlapping pixels should take about 1 seconds for every 100 pixels. So, if you have two images with 100 pixels each, overlapping pixels should take about 2 seconds. 

Performing the fit - This is the most variable section. Some reference times are provided here, giving number of pixels, number of images, choice of simulation code, use of one SED for all pixels, and number of parameters to fit for. Keep in mind that the choice of initial guess and other choices may influence these results. 

Simulating best-fit image – Should be on the order of seconds to a minute. Longer than a minute is cause for concern. 
...
ised_dir (string) – path to the ised files to be used. Requires the user to choose the library and the IMF. 
...
provenance_tag (string) – the provenance string to use if automatically retrieving data
...
use_bayes (bool, optional) – choice to use Bayesian statistics for a more complete exploration of the parameter space and uncertainty. Default is False. Currently not supported. If this is True, the best fit returned will be the median of the posterior with uncertainty and the best fit images will be made with these values. To return the complete chains for your own analysis, set full_bayes to be True in the output parameters

niter (int, optional) – number of iterations to use when running Bayesian code. Only used if use_bayes is True. Default is 10^5.

nwalkers (int, optional) – number of walkers to use when running Bayesian code. Only used if use_bayes is True. Default is 8. 

sim_code (string) – choice of simulation code, either BC03 (also known as GALAXEV) or FSPS. BC03 runs faster but allows for fewer inputs and outputs. FSPS is slower but allows for a lot of options for both input and output. BC03 does not permit the user to select which parameters to fit for as the number of parameters is inherently small. FSPS requires the user to select which parameters to fit for as the possible number of fit parameters per pixel can be very high. 

one_sed (bool, optional) – choice to allow each pixel to have a unique SED or not. If True, every pixel is assumed to have the same SED. This means the SED simulation is only done once per pass, saving time. If False, each pixel gets a unique SED. This takes longer, but allows the galaxy to vary over space

buffer (int, optional) – buffer around the segmentation map to test for overlapping pixels when translating between different images. Default is 1, which should be plenty, but can be changed to be larger if faint or edge pixels are a concern. 

H0 (float, optional) – Hubble constant value to use in calculating distance from redshift. Cosmology is always flat LambdaCDM. Default is 70.

OmegaM (float, optional) – Matter density to use in calculating distance from redshift. Cosmology is always flat LambdaCDM. Default is 0.3. 

bc03_params – parameters required to perform a BC03 simulation. These must all be provided. For more details on these parameters, see the BC03 documentation here.

library (string) – library to use, e.g. “BaSeL”

metallicity (string) – metallicity to use, e.g. “m82”

imf (string) – initial mass function to use, e.g. “kroup”

dust (bool, optional) – choice to fit for dust or not. Default is False. 

sfh (int) – star formation history choice. Must be 0, 1, -1, 2, 3, 4, 5, or 6. SFH 7 is not currently supported. If SFH is 1 or -1, dust recycling is an option (see below). If SFH is 6, provide a list of SED files matching the number of pixels if one_sed is False, and one file if one_sed is True. 
